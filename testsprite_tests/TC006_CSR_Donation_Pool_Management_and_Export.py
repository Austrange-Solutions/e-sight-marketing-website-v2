...
async def run_test():
    ...
    try:
        ...
        # Click on Login link to access login page
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=CSR Donation Entry Successful').first).to_be_visible(timeout=3000)
        except AssertionError:
            raise AssertionError("Test case failed: Admin abilities to add, filter, audit, edit CSR donations and export donation analytics as Excel could not be verified. The expected confirmation text 'CSR Donation Entry Successful' was not found on the page, indicating failure in donation management functionality.")
