...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to admin dashboard or settings page to update foundation fee percentages.
        frame = context.pages[-1]
        # Click on Profile or admin menu to find admin dashboard or settings link
        elem = frame.locator('xpath=html/body/div[2]/nav/div/div/div[2]/a[7]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Try to access admin dashboard or foundation fee configuration page directly via URL or report issue if not accessible.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Foundation Fee Updated Successfully').first).to_be_visible(timeout=3000)
        except AssertionError:
            raise AssertionError("Test failed: Admins could not update foundation fee percentage configurations or changes did not apply immediately to new donations as required by the test plan.")
