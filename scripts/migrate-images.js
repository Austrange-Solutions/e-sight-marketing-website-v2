...
async function migrateProductImages() {
  try {
    ...
    for (const product of products) {
      try {
        ...
        if (
          ...
          console.log(
            `⏭️  Skipping ${product.name} - already has CloudFront URL`
          );
          skippedCount++;
          continue;
        }
        ...
        if (product.image && mongoose.Types.ObjectId.isValid(product.image)) {
          ...
          if (uploadedImage && uploadedImage.cloudFrontUrl) {
            ...
            console.log(
              `✅ Migrated ${product.name}: ${uploadedImage.cloudFrontUrl}`
            );
            migratedCount++;
          } else {
          ...
          console.log(
            `⚠️  Product ${product.name} has unusual image format: ${product.image}`
          );
        }
      } catch (error) {
        console.error(
    ...
    console.log("\n💡 Next steps:");
    console.log(
      "   1. Update your upload APIs to store CloudFront URLs directly in products"
    );
    console.log(
      "   2. Consider keeping UploadedImage collection for audit/cleanup purposes"
    );
