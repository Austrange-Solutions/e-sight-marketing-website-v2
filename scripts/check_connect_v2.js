...
files.forEach(f => {
  ...
  const hasAwaitDbConnect = content.includes('await dbConnect(') || content.includes('await dbConnect()');
  const hasAwaitConnect = content.includes('await connect(') || content.includes('await connect()');
  const hasTopLevelConnect = content.includes('\nconnect();') || content.includes('\nconnect();\r');
  const hasAwaitEnsure = content.includes('await __ensureConnect()');
  const hasAwaitDbConnectAlt = content.includes('await dbConnect;');

  if (!(hasAwaitDbConnect || hasAwaitConnect || hasTopLevelConnect || hasAwaitEnsure || hasAwaitDbConnectAlt)) {
