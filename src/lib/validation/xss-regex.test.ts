...
describe("XSS Detection - Script Tags", () => {
  ...
  test("should detect script tags with attributes", () => {
    expect(containsXSS('<script src="http://evil.com/xss.js"></script>')).toBe(
      true
    );
    expect(containsXSS('<script language="javascript">alert(1)</script>')).toBe(
      true
    );
  });
...
describe("XSS Detection - Data URIs", () => {
  ...
  test("should detect base64 encoded data URIs", () => {
    expect(
      containsXSS("data:text/html;base64,PHNjcmlwdD5hbGVydCgxKTwvc2NyaXB0Pg==")
    ).toBe(true);
  });
});
...
describe("XSS Detection - Dangerous Tags", () => {
  test("should detect iframe", () => {
    expect(containsXSS('<iframe src="javascript:alert(1)"></iframe>')).toBe(
      true
    );
  });
  ...
  test("should detect object", () => {
    expect(containsXSS('<object data="javascript:alert(1)"></object>')).toBe(
      true
    );
  });
...
describe("XSS Detection - CSS Attacks", () => {
  test("should detect expression()", () => {
    expect(containsXSS('<div style="width:expression(alert(1))"></div>')).toBe(
      true
    );
  });
  ...
  test("should detect @import", () => {
    expect(
      containsXSS('<style>@import"http://evil.com/xss.css";</style>')
    ).toBe(true);
  });
});
...
describe("XSS Detection - Dangerous Functions", () => {
  ...
  test("should detect document.write", () => {
    expect(containsXSS('document.write("<script>alert(1)</script>")')).toBe(
      true
    );
  });
...
describe("XSS Detection - Real World Examples", () => {
  test("should detect payload from screenshots", () => {
    ...
    expect(
      containsXSS("<SCRIPT>alert(String.fromCharCode(88,83,83))</SCRIPT>")
    ).toBe(true);
    expect(containsXSS("<marquee onstart=\"javascript:alert('Y')>\">")).toBe(
      true
    );
    expect(containsXSS('<a href="javas\\0\\0cript:javascript:alert(1)">')).toBe(
      true
    );
  });
