...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Link href={productUrl} className="block h-full">
      <Card
        className={`flex flex-col h-full w-full relative overflow-hidden border-border hover:border-primary transition-all duration-300 transform hover:scale-[1.02] ${isOutOfStock ? "bg-muted" : ""}`}
      >
        ...
        {isOutOfStock && (
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20">
            <Badge
              variant="destructive"
              className="px-3 py-2 sm:px-4 sm:py-2 md:px-6 md:py-3 font-bold text-xs sm:text-sm md:text-base shadow-lg"
            >
              OUT OF STOCK
            </Badge>
          </div>
        ...
        <div
          className={`relative w-full h-48 sm:h-56 md:h-64 lg:h-72 xl:h-80 ${isOutOfStock ? "filter grayscale opacity-70" : ""}`}
        >
          <Image
            src={product.image}
            alt={product.name}
          ...
          <div className="mb-3 sm:mb-4 md:mb-5 flex-grow">
            ...
            <ul className="space-y-1 text-left">
              {product.details && product.details.length > 0 ? (
                product.details.slice(0, 3).map((point, index) => (
                  <li
                    key={index}
