  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-white p-8 rounded-2xl shadow-lg mb-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-6">Check Ticket Status</h2>
        <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4">
          ...
          >
            {loading ? "Searching..." : <><Search className="w-5 h-5" /> Check Status</>}
          </button>
        </form>
      </div>
            ...
            <div key={ticket._id} className="bg-white rounded-2xl shadow-lg overflow-hidden">
              <div className="bg-gray-50 px-8 py-6 border-b border-gray-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  ...
                  <p className="text-gray-500 text-sm mt-1">ID: <span className="font-mono text-gray-700">{ticket.ticketId}</span></p>
                  <p className="text-gray-400 text-xs mt-1">Created: {new Date(ticket.createdAt).toLocaleDateString()}</p>
                </div>
                <div className={`flex items-center gap-2 px-4 py-2 rounded-full font-medium ${getStatusColor(ticket.status)}`}>
              ...
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Issue Information</h4>
                  <div className="space-y-4">
                    <div>
                      <p className="text-sm text-gray-500">Category</p>
                      <p className="font-medium text-gray-900">{ticket.problemCategory}</p>
                    </div>
                    {ticket.customProblem && (
                      <div>
                        <p className="text-sm text-gray-500">Specific Issue</p>
                        <p className="font-medium text-gray-900">{ticket.customProblem}</p>
                      </div>
                    )}
                    <div>
                ...
                <div>
                  <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Updates & Response</h4>
                  {ticket.adminResponse ? (
                    <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg">
                      <p className="text-xs font-bold text-blue-600 uppercase mb-2">Support Team Response</p>
                    ...
                    <div className="text-center py-8 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                      <p className="text-gray-500">No response from support team yet.</p>
                      <p className="text-sm text-gray-400 mt-1">We typically respond within 24-48 hours.</p>
                    </div>
                  )}
                </div>
              </div>
