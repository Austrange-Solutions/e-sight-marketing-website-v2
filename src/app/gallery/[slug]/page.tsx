  ...
  try {
    ...
    if (!event) {
      ...
      if (/^[0-9a-fA-F]{24}$/.test(maybeId)) {
        event = await EventModel.findById(maybeId)
          .populate("thumbnailImage")
          .populate("galleryImages")
          .lean();
      }
    ...
    for (let i = 0; i < imageKeys.length; i++) {
      ...
      const url =
        signedMap.get(key) ||
        `${process.env.NEXT_PUBLIC_CLOUDFRONT_DOMAIN || process.env.CLOUDFRONT_DOMAIN}/${key}`;
      if (url) {
    ...
    const thumbUrl =
      (event.thumbnailImage as any)?.fileUrl ||
      (event.thumbnailImage as any)?.cloudFrontUrl ||
      undefined;
