...
export async function POST(
  ...
  try {
    ...
    if (!userId) {
      console.warn(
        "[POST_REVIEW_INVALID_USER] Could not resolve ObjectId for user",
        { candidateId, email: (userData as any).email }
      );
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }
    ...
    try {
      sanitizedComment = validateAndSanitizeWithWordLimit(
        String(comment).trim(),
        {
          fieldName: "comment",
          maxWords: 150,
...
export async function DELETE(
  ...
  try {
    ...
    if (String(review.product) !== String(productId)) {
      return NextResponse.json(
        { error: "Invalid product for this review" },
        { status: 400 }
      );
    }
