...
const DocumentsPage = () => {
  ...
  const fetchResources = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/resources?category=documents&limit=100');
      ...
      <div className="bg-white border-b border-gray-200 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-4 text-[#0C5277]">Documents</h1>
            <p className="text-lg text-[#1B9BD8] max-w-3xl mx-auto">
              Access important organizational documents and resources
            </p>
          </div>
        </div>
      </div>
